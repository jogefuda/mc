...
static RSA* PEM_load_pubkey_from_str(const char* publicKeyStr) {
    ...
    RSA* rsa;
    if (0 == (rsa = PEM_read_bio_RSA_PUBKEY(bio, NULL, NULL, NULL)))
        fprintf(stderr, "ERROR: Could not load PUBLIC KEY!  PEM_read_bio_RSA_PUBKEY FAILED: %s\n", ERR_error_string(ERR_get_error(), NULL));
...
RSA *DER_load_pubkey_from_str(struct buffer *arr) {
    /* Encode DER public key with Base64 */
    gnutls_datum_t raw, b64;
    raw.data = arr->b_data;
    raw.size = arr->b_size;
    gnutls_base64_encode2(&raw, &b64);

    /* Convert DER public key to PEM format */
...
int RSA_encrypt_with_pubkey(RSA *rsa, struct buffer *in, struct buffer *out) {
    ...
    /* Scale the buffer size to keysize */
    if (M_FAIL == inc_buffer_if_not_enough(out, keysize)) {
        return -1;
    }

    /* Encrypt with public key */
    ret = RSA_public_encrypt(in->b_size, (unsigned char *)in->b_data, (unsigned char *)out->b_data, rsa, RSA_PKCS1_PADDING);
    out->b_size += keysize;
...
/* Generate n bytes of random data for secret key */
...
EVP_CIPHER_CTX *aes_cipher_init(const char *key, const char *iv, int enc) {
    ...
    /* Init cipher context with key and iv generated by client */
    if (0 == (ret = EVP_CipherInit(ctx, EVP_aes_128_cfb8(), key, iv, enc))) {
